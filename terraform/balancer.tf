...
resource "yandex_lb_target_group" "k8s_web_targets" {
  ...
  target {
    ...
    address   = yandex_compute_instance.worker[0].network_interface[0].ip_address  # приватный IP
  }
  target {
...
# Сетевая группа целей для балансиравщика API (мастер-ноды)
resource "yandex_lb_target_group" "k8s_api_targets" {
  ...
  target {
    subnet_id = yandex_vpc_subnet.k8s_subnet[0].id
    address   = yandex_compute_instance.master[0].network_interface[0].ip_address  # приватный IP
  }
  target {
    subnet_id = yandex_vpc_subnet.k8s_subnet[1].id
...
# Сетевой балансировщик нагрузки для API Kubernetes
...
# Application Load Balancer с автоматическим выделением одного публичного IP
resource "yandex_alb_load_balancer" "k8s_web_lb" {
  ...
  listener {
    ...
    endpoint {
      address {
        external_ipv4_address {}  # Публичный IP выделяется автоматически без указания ключа address
      }
      ports = [80, 443]
    }
    http {
